"""
SpectraSoft — Job X: Content Analysis

This is the final routine analysis job.

Workflow:
1. User enters sample name.
2. User presses Start one or more times to collect burns.
3. AnalysisWorker returns pre-correction intensities.
4. CorrectionEngine applies:
      Page 4 drift correction
      Page 5 working curve
      Page 6 matrix correction
      Page 7 master correction
      Page 9 purity calculation
5. Page 8 display rules are used for order/formatting.
6. Table shows Element | AVE | N=1 | N=2 | ...

Important:
- Job X displays final chemical content values.
- Job X does not file calibration coefficients.
- Job X uses all registered analytical group data.
"""
...
class JobXRunPage(QWidget):
    """Job X: Content Analysis page."""

    def __init__(self, main_window, group_id: int, group_name: str, job_type: str = "X"):
        super().__init__()
    ...
    def _build_ui(self):
        ...
        self.title_label = QLabel(f"Job X: Content Analysis - {self.group_name}")
        ...
        # ── Status Label ─────────────────────────────────────────────────
        self.status_label = QLabel("Ready. Enter sample name and press Start.")
    ...
    def _load_group_data_for_correction(self) -> dict:
        """
        Load all correction/configuration pages needed for Job X.
        """
        session = get_session()
    ...
    def _apply_content_pipeline(self, intensities: dict) -> dict:
        """
        Apply complete Job X correction pipeline.
        """
    ...
    def _display_element_names(self) -> list:
        """
        Determine display order using Page 8.

        If Page 8 has any ORDER > 0:
            show ORDER > 0 sorted by ORDER.
    ...
    def _format_value_for_element(self, elem: str, value) -> str:
        """
        Format one result value using Page 8 DECI/MAGN.
        """
        group_data = self._load_group_data_for_correction()
        page8 = group_data.get("page_08_display", {})
        ...
        if isinstance(page8, dict) and isinstance(page8.get("rows"), list):
            for row in page8["rows"]:
                ...
                if name.upper() == elem.upper():
                    try:
                        deci = int(float(str(row.get("deci", "0")).strip() or "0"))
                    except (TypeError, ValueError):
                        deci = 0

                    try:
                        magn = int(float(str(row.get("magn", "0")).strip() or "0"))
                    except (TypeError, ValueError):
                        magn = 0
    ...
    def _update_table(self):
        if not self.element_names:
            ...
            self.status_label.setText(
                "No elements configured. Complete Page 3 before running Job X."
            )
            return
    ...
    def _on_start(self):
        ...
        if not self.element_names:
            QMessageBox.warning(
                ...
                "Please complete Page 3 before running Job X."
            )
            return
        ...
        self.status_label.setText(f"Starting content analysis for {sample}...")
        self.progress_bar.setValue(0)
    ...
    def _on_result(self, results: dict):
        """
        Receive pre-correction intensities and calculate final content.
        """
    ...
    def _generate_html_report(self) -> str:
        title = f"SpectraSoft Content Analysis Report - {self.group_name}"
